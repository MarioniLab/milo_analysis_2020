...
In order to define a representative subset of neighbourhoods that span the whole k-NN graph, we implement a previously adopted algorithm to sample the index cells in a graph [@gutTrajectoriesCellcycleProgression2015; @settyWishboneIdentifiesBifurcating2016]. 
...
_Milo_ builds upon the framework for differential abundance testing implemented by _Cydar_ [@lunTestingDifferentialAbundance2017]. In this section, we briefly describe the statistical model and adaptations to the k-NN graph setting. 
...
To control for multiple testing, we adapt the Spatial FDR method introduced by $Cydar$ [@lunTestingDifferentialAbundance2017]. The Spatial FDR can be interpreted as the proportion of the union of neighbourhoods that is occupied by false-positive neighbourhoods. This accounts for the fact that some neighbourhoods are more densely connected than others. To control spatial FDR in the k-NN graph, we apply a weighted version of the Benjamini-Hochberg (BH) method. Briefly, to control for FDR at some threshold $\alpha$ we reject null hypothesis $i$ where the associated p-value is less than the threshold
$$
