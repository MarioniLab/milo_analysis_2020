...
thymus.mylo <- buildNhoodGraph(thymus.mylo, overlap=30)
...
thymus.arrow <- ggplot(orig.da, aes(x=Cluster, y=start, xend=Cluster, yend=end, colour=Direction)) +
    ...
    coord_flip() +
    theme_bw() +
    scale_colour_manual(values=c("blue", "red"), breaks=c("Up", "Down"),
...
thymus.markers <- findNhoodMarkers(thymus.mylo, thymus.res, 
                                   ...
                                   subset.row=keep.genes,
                                   da.fdr=0.1, overlap=10, compute.new=FALSE, gene.offset=TRUE)
