...
(plot_thresh_hist(linear_res_df) + ggtitle("1D trajectory", subtitle="") |
plot_thresh_hist(branch_res_df) + ggtitle("Branching trajectory", subtitle="") |
plot_thresh_hist(embryo_res_df) + ggtitle("Mouse Gastrulation", subtitle="")) +
  ggsave(paste0(figdir, "pC1_full_hist_quantiles.pdf"), width=16, height = 5)
```
...
linear_res_df %>%
  group_by(id) %>%
  mutate(max_pC1 = 1-min(true_prob)) %>%
  ungroup() %>%
  filter(max_pC1==0.7) %>%
  plot_p_hist() +
  xlim(0.5,1) +
...
outdir <- "/nfs/team205/ed6/data/milo_benchmark/cluster3x/"
linear_sce <- readRDS("~/data/milo_benchmark/cluster3x_data_bm.RDS")
indir <- "~/data/milo_benchmark/synthetic_data/"
...
lfc_comparison <- function(milo_out, meld_out){
  ...
  lfc_df <- nh_meld %>%
        mutate(milo_LFC=milo_out$logFC,
         ...
         true_labels=ifelse(true_prob < 1-embryo_thresh, "NegLFC", "NotDA"),
...
DA_pop_size <- sapply(coldata_files, function(x) read_csv(x) %>% pull(Condition1_prob) %>% {sum(. > embryo_thresh)})
DA_pop_size_df <- 
  coldata_files %>% 
  ...
  purrr::reduce(rbind) %>%
  data.frame() %>%
  dplyr::rename(pop=X1, enr=X2, seed=X3, logit=X4) %>%
...
pl_A +
  ggsave(paste0(figdir,"Sfig_signal2noise_pC1.pdf"), height = 6, width = 9) +
  ggsave(paste0(figdir,"Sfig_signal2noise_pC1.png"), height = 6, width = 9)
```
...
read_input_data <- function(embryo_sce, indir, pop, enr, seed, batchEffect, run_umap=TRUE){
  pca_file <- list.files(indir, pattern=paste0(pop,".+", "enr",enr,'_seed', seed, '.+batchEffect',batchEffect,".pca.csv"),
                         full.names = TRUE)
  coldata_file <- list.files(indir, pattern=paste0(pop,".+", "enr",enr,'_seed', seed,".coldata.csv"),
                         full.names = TRUE)
...
## Quantile threshold
q_thresh <- quantile(sce$Condition1_prob)[4]
data.frame(pC1 = sce$Condition1_prob) %>%
  mutate(true_labels=ifelse(sce$Condition1_prob > q_thresh, 'NegLFC', 'NotDA') ) %>%
  ggplot(aes(pC1, fill=true_labels)) + geom_histogram(bins=100) +
  theme_bw(base_size=24) +
  xlab("P(C1)") + ylab("# cells") +
  geom_vline(xintercept = q_thresh, color="black", linetype=2, size=1) +
  scale_fill_manual(values = c(NegLFC="red", NotDA="grey"), labels=c(NegLFC='cell in DA region', NotDA="Not DA"), name="") +
  theme(axis.text.y=element_blank(), axis.ticks.y=element_blank()) +
  ggsave("~/mount/gdrive/milo/Figures/benchmark_v2/bm_design_qThresh.pdf", width=7, height = 4)

p4 <- umap_bath_df %>%
    dplyr::rename(UMAP1=X1, UMAP2=X2) %>%
    mutate(true_labels=ifelse(sce$Condition1_prob > 0.6, 'NegLFC', 'NotDA') ) %>%
    ggplot(aes(UMAP1, UMAP2, color=true_labels)) + 
    geom_point_rast(size=0.1, raster.dpi = 500) +
    scale_color_manual(values = c(NegLFC="red", NotDA="grey"), labels=c(NegLFC='cell in DA region', NotDA="Not DA"), name="") +
    theme_classic(base_size=16) +
     guides(color=guide_legend(override.aes = list(size=2))) +
     theme(axis.ticks = element_blank(),
           axis.text = element_blank(),
           legend.position = c(0.85,0.95), legend.background = element_blank()) +
     xlab("UMAP1") + ylab("UMAP2") 
    # ggsave("~/mount/gdrive/milo/Figures/benchmark_v2/bm_design_qThresh_umap.pdf", width=7, height = 4)
...
(p1 + p2 + p3) + des_pl + p4 +
  plot_layout(widths=c(1,1,1,0.2, 1), ncol=5) +
  ggsave("~/mount/gdrive/milo/Figures/benchmark_v2/bm_example.pdf", height = 4, width = 14) +
  ggsave("~/mount/gdrive/milo/Figures/benchmark_v2/bm_example.png", height = 4, width = 14)
