...
(thymus.arrow + thymus.bee +
    ...
    ggsave("~/Dropbox/Milo/figures/Thymus_ResByCluster.pdf",
       height=6.15, width=7.95, useDingbats=FALSE)
```
...
thymus.markers <- findNhoodMarkers(thymus.mylo, thymus.res, 
                                   subset.nhoods=thymus.res$Cluster %in% c("Intertypical TEC"),
                                   subset.row=keep.genes, return.groups=TRUE,
                                   da.fdr=0.1, overlap=0, compute.new=FALSE, gene.offset=FALSE)
...
```{r, fig.height=7.95, fig.width=7.55, warning=FALSE}
plot.select <- c("Cd40", "Ccl21a", "Ly6a", "Stat1", "Il18r1", "Stat4", "Aff3", "Ptpn18")

select.markers <- unique(c(thymus.markers[thymus.markers$adj.P.Val_1 < 0.01 & thymus.markers$logFC_1 > 1, ]$GeneID, 
...
ggsave("~/Dropbox/Milo/figures/MouseThymus_iTEC_heatmap.pdf",
       height=7.95, width=7.55, useDingbats=FALSE)

ggsave("~/Dropbox/Milo/figures/MouseThymus_iTEC_heatmap.png",
       height=7.95, width=7.55, dpi=90)
```
...
de.genes <- unique(c(thymus.markers[thymus.markers$adj.P.Val_1 < 0.01, ]$GeneID,
...
thymus.markers <- findNhoodMarkers(thymus.mylo, thymus.res, 
                                   ...
                                   da.fdr=0.1, overlap=0, compute.new=FALSE, gene.offset=FALSE)
# thymus.markers$GeneID <- rownames(thymus.mylo):
thymus.groups <- thymus.markers$groups
thymus.markers <- thymus.markers$dge
thymus.markers$GeneID <- keep.genes
```
...
```{r, warning=FALSE, message=FALSE}
droplet.sce <- readRDS("~/Dropbox/Milo/simulations/data/ThymusDroplet_SCE.RDS")
# droplet.sce <- scuttle::logNormCounts(droplet.sce)
```

I will need to batch-correct the 2 data sets together first, which I'll use MNN for, once the cells are projected into the same PC space.

```{r, message=FALSE}
common.genes <- intersect(rowData(thymus.mylo)$ensembl_gene_id, rownames(droplet.sce))
rownames(thymus.mylo) <- rowData(thymus.mylo)$ensembl_gene_id
rowData(droplet.sce) <- rowData(droplet.sce)[, c("ensembl_gene_id", "external_gene_name")]
colnames(rowData(droplet.sce)) <- c("ensembl_gene_id", "symbol")

multibatch.pca <- multiBatchPCA(droplet.sce[common.genes, ], thymus.mylo[common.genes, ], subset.row=common.genes)
```
