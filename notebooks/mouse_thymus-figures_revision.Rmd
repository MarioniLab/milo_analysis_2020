...
ggsave("~/Dropbox/Milo/revision/figures/MouseThymus_FR_singleCells.png",
...
ggsave(aligned.plots[[1]], filename="~/Dropbox/Milo/revision/figures/MouseThymus_UMAP_singleCells.png",
       height=2.95, width=6.15, dpi=300)

ggsave(aligned.plots[[1]], filename="~/Dropbox/Milo/revision/figures/MouseThymus_UMAP_singleCells.pdf",
       height=2.95, width=6.15, useDingbats=FALSE)


ggsave(aligned.plots[[2]], filename="~/Dropbox/Milo/revision/figures/MouseThymus_UMAP_singleCells-Age.png",
       height=2.95, width=6.15, dpi=300)

ggsave(aligned.plots[[2]], filename="~/Dropbox/Milo/revision/figures/MouseThymus_UMAP_singleCells-Age.pdf",
       height=2.95, width=6.15, useDingbats=FALSE)
```
...
ggsave("~/Dropbox/Milo/revision/figures/MouseThymus_volcano.pdf",
       height=2.95, width=5.15, useDingbats=FALSE)
```

We want to make sure that there is no indication of any global shifts due to confounding between the numbers of cells collected and each age. We can 
do this by visualising the DA testing results with an MA plot.

```{r, warning=FALSE, message=FALSE, fig.height=2.55, fig.width=5.95}
plotNhoodMA(thymus.res, alpha=0.1) +
  ggsave("~/Dropbox/Milo/revision/figures/Thymus_MAplot.png",
         height=2.55, width=5.95, dpi=300)
```

The two horizontal lines represent the expect log fold change under the null hypothesis (grey line) and the observed median log-fold change 
(purple line). Ideally we want these two lines to be pretty close together, which they are here.
...
ggsave("~/Dropbox/Milo/revision/figures/MouseThymus_UMAP_nhoods-logFC.png",
       height=6.95, width=7.55, dpi=300)

ggsave("~/Dropbox/Milo/revision/figures/MouseThymus_UMAP_nhoods-logFC.pdf",
...
ggsave("~/Dropbox/Milo/revision/figures/MouseThymus_UMAP_nhoods-cluster.png",
       height=5.95, width=6.95, dpi=300)

ggsave("~/Dropbox/Milo/revision/figures/MouseThymus_UMAP_nhoods-cluster.pdf",
...
```{r, warning=FALSE, message=FALSE, fig.height=6.15, fig.width=6.55}
...
thymus.bee <- ggplot(mapping=aes(x=Cluster, y=logFC)) +
    ...
    theme(strip.text.y =  element_text(angle=0, colour='black', size=16),
...
(thymus.arrow + thymus.bee +
    ...
    ggsave("~/Dropbox/Milo/revision/figures/Thymus_ResByCluster.pdf",
       height=6.15, width=4.95, useDingbats=FALSE)
```
...
```{r, nhood-expression, message=FALSE, warning=FALSE}
thymus.mylo <- calcNhoodExpression(thymus.mylo) # This make plotting faster later.
```


```{r, define-nhood-groups, message=FALSE, warning=FALSE}
...
thymus.res <- groupNhoods(thymus.mylo, thymus.res, da.fdr=0.1, overlap=5,
                          max.lfc.delta=0.1,
                          subset.nhoods=thymus.res$Cluster %in% c("Intertypical TEC", "Mature mTEC"),
                          compute.new=TRUE)
```

We can overlay the nhood groups onto the UMAP to see how they correspond to the clusters in the original manuscript.

```{r, plot-nhood-groups, warning=FALSE, message=FALSE, fig.height=3.95, fig.width=8.95}
plotNhoodGroups(thymus.mylo, thymus.res) +
  ...
  ggsave("~/Dropbox/Milo/revision/figures/Mouse_Thymus_nhoodGroups.pdf",
         height=3.95, width=8.95, useDingbats=FALSE) +
  NULL
```

```{r, tabulate-nhood-groups, warning=FALSE, message=FALSE}
...
```{r, nhood-markers, warning=FALSE, message=FALSE}
thymus.markers <- findNhoodGroupMarkers(thymus.mylo, thymus.res,
                                        subset.groups=c("3", "4", "5"),
                                        #subset.nhoods=thymus.res$NhoodGroup %in% c("3", "4", "5"),
                                        #subset.nhoods=thymus.res$Cluster %in% c("Intertypical TEC", "Mature mTEC"),
...
```{r, expression-heatmap, fig.height=7.95, fig.width=7.55, warning=FALSE}
plot.select <- c("Cd40", "Ccl21a", "Ly6a", "Stat1", "Il18r1", "Stat4", "Aff3", "Ptpn18")

select.markers <- unique(c(thymus.markers[thymus.markers$adj.P.Val_5 < 0.05 & thymus.markers$logFC_5 > 1, ]$GeneID, 
...
ggsave("~/Dropbox/Milo/revision/figures/MouseThymus_iTEC_heatmap.pdf",
       height=6.15, width=7.55, useDingbats=FALSE)

ggsave("~/Dropbox/Milo/revision/figures/MouseThymus_iTEC_heatmap.png",
       height=6.15, width=7.55, dpi=90)
```
...
```{r, pathway-enrichment, warning=FALSE, message=FALSE}
dbs <- c("GO_Biological_Process_2018")
de.genes <- unique(c(thymus.markers[thymus.markers$adj.P.Val_5 < 0.05, ]$GeneID))
