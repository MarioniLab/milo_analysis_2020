...
```{r}
sim_milo <- makeNeighbourhoods(sim_milo, prop = 0.1, k=20, d=30, refined = FALSE)
plotNeighborhoodSizeHist(sim_milo)
```
...
milo_results <- testNeighbourhoods(sim_milo, ~ 1 + condition, data = design_df,
...
nhIndex <- unlist(neighbourhoodIndex(sim_milo))
nhSize <- sapply(neighbourhoods(sim_milo), length)
...
```{r, warning=FALSE, message=FALSE, echo=TRUE}
bind_rows(
  data.frame(neighborhood_size = unlist(lapply(neighbourhoods(sim_milo), length)), sampling="random"),
  data.frame(neighborhood_size = unlist(lapply(neighbourhoods(sim_milo_ref), length)), sampling="refined")
  ) %>%
...
for (x in seq(0.01,0.5, by = 0.05)) {
  for (i in 1:3){
    random_nh  <- neighbourhoods(makeNeighbourhoods(sim_milo, prop=x, k=20, d=30, refined = FALSE, seed=42 + i))
    refined_nh <- neighbourhoods(makeNeighbourhoods(sim_milo, prop=x, k=20, d=30, refined = TRUE, seed=42 + i))
...
```{r}
sim_milo_ref <- countCells(sim_milo_ref, data = data.frame(colData(sim_milo_ref)), samples = "sample")

res_rand <- testNeighbourhoods(sim_milo, ~ 1 + condition, data = design_df, fdr.weighting = "k-distance")
