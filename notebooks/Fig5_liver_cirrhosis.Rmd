...
plotNhoodGraphDA(liver_milo, milo_res, alpha = 0.05, size_range=c(2,6))
```
...
```{r}
set.seed(42)
milo_res_endogroups <- groupNhoods(liver_milo, milo_res, max.lfc.delta = 0.5, overlap = 1)
```

Look at groups in endothelial cells 

```{r}
plotDAbeeswarm(milo_res_endogroups, group.by = "NhoodGroup", alpha = 0.1, 
               subset.nhoods = milo_res_endogroups$annotation_lineage=="Endothelia")
```

groups 54 and 70 contain most of the DA neighbourhoods
...
fig4_bright1 <- ((endo_umap + endo_gr + endo_gr_groups ) + 
...
pl3 <- fig4_bbright$data %>%
  ggplot(aes(logFC_rank, 1,fill=as.factor(NhoodGroup))) +
  geom_tile() +
...
<!-- ```{r} -->
<!-- ca_signature <- m_t2g %>% -->
<!--   filter(str_detect(gs_name, "POSITIVE_REGULATION_OF_CALCIUM_ION_TRANSPORT"))  %>% -->
<!--   filter(gene_symbol %in% rownames(liver_milo)) -->

<!-- marker.df.chol %>% -->
<!--   filter(GeneID %in% ca_signature$gene_symbol & adj.P.Val_49 < 0.1) %>% -->
<!--   ggplot(aes(adj.P.Val_49)) + -->
<!--   geom_histogram() -->
<!-- ``` -->
...
plot_grid( frac_hist + theme_bw(base_size=18),
  go_endo_up+ theme(plot.title = element_text(hjust = 1),
                 ...
                 label_size = 18,
                 ncol=1,
                 rel_heights = c(1.2,2,2), rel_widths = c(1,2,2),
                labels = c("A", "B","C")
  ) +
