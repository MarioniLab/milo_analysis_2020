...
trajectory simulation with 5000 cells.  The methods against which I will make this comparison are:
...
n.dim <- 20
k <- 50
sim.data <- readRDS("~/Dropbox/Milo/simulations/data/Trajectory_Ncells5000_M1DA_simpleSim.RDS")
...
```{r, warning=FALSE, message=FALSE, fig.height=4.15, fig.width=5.15}
set.seed(42)
...
ggplot(sim.fr.df, aes(x=V1, y=V2)) +
    ...
    guides(colour=guide_legend("Ground.Truth"))
```
...
sim.mylo <- makeNhoods(sim.mylo, k=k, d=n.dim, prop=0.5, refined=TRUE)
...
sim.dist <- neighborDistances(sim.cydar, neighbors=50, as.tol=TRUE)
boxplot(sim.dist)
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
sim.cydar <- cydar::countCells(sim.cydar, tol=1.5, filter=0, downsample=3)
...
`Cydar` finds ~200 DA hyperspheres in this example.
...
sim.da_regions <- getDAregion(X=reducedDim(sim.mylo)[, 1:n.dim],
                              ...
                              labels.2=sim.meta$cell_id[sim.meta$Condition %in% c("B")],
                              size=1,
                              resolution=0.01, plot.embedding=as.matrix(sim.fr.df[, c("V1", "V2")]))
...
```{r, warning=FALSE}
true.da.cells <- sim.meta$cell_id[sim.meta$group_id %in% "M1"]
...
walktrap.fdr <- daseq.confuse.list$FP/(walktrap.confuse.list$TP + walktrap.confuse.list$FP)
daseq.fdr <- milo.confuse.list$FP/(daseq.confuse.list$TP + daseq.confuse.list$FP)
...
walktrap.fnr <- daseq.confuse.list$FN/(walktrap.confuse.list$TP + walktrap.confuse.list$FN)
daseq.fnr <- milo.confuse.list$FN/(daseq.confuse.list$TP + daseq.confuse.list$FN)
...
walktrap.fpr <- daseq.confuse.list$FP/(walktrap.confuse.list$FP + walktrap.confuse.list$TN)
daseq.fpr <- milo.confuse.list$FP/(daseq.confuse.list$FP + daseq.confuse.list$TN)
...
walktrap.for <- daseq.confuse.list$FN/(walktrap.confuse.list$FN + walktrap.confuse.list$TN)
daseq.for <- milo.confuse.list$FN/(daseq.confuse.list$FN + daseq.confuse.list$TN)
...
walktrap.tpr <- daseq.confuse.list$TP/(walktrap.confuse.list$TP + walktrap.confuse.list$FN)
...
walktrap.tnr <- daseq.confuse.list$TN/(walktrap.confuse.list$TN + walktrap.confuse.list$FP)
...
walktrap.npv <- daseq.confuse.list$TN/(walktrap.confuse.list$TN + walktrap.confuse.list$FN)
