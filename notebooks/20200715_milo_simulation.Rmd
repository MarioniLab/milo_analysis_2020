---
title: "Milo - simulated trajectory graphs"
...
<!-- # ```{r} -->
<!-- # ## Compute UMAP for all cells -->
<!-- # sim_umap <- uwot::umap(reducedDim(sim_milo), n_neighbors = 20, metric = "cosine") -->
<!-- # reducedDim(sim_milo, "UMAP") <- sim_umap -->

<!-- ## Take median of neighbourhoods -->
<!-- nhMed <- t(sapply(neighbourhoods(sim_milo), function(x) colMedians(reducedDim(sim_milo)[x,]))) -->
<!-- umap_nhMed <- uwot::umap(neighbourhoodsMed, n_neighbors=20, metric='cosine', -->
<!--            init=reducedDim(sim_milo[,unlist(neighbourhoodIndex(sim_milo))], "UMAP"), min_dist=0.1) -->
<!-- colnames(umap_nhMed) <- c("UMAP_1", "UMAP_2") -->

<!-- milo_results_viz <- cbind(milo_results, umap_nhMed)  -->
<!-- milo_results_viz %>% -->
<!--   arrange(abs(logFC)) %>% -->
<!--   mutate(logFC = ifelse(SpatialFDR > filter_alpha, NA, logFC)) %>% -->
<!--   ggplot(aes(UMAP_1, UMAP_2)) + -->
<!--   geom_point(aes(color=logFC)) + -->
<!--   scale_color_gradient2(midpoint = 0, high="red", low="blue", name="log-FC") + -->
<!--   theme_classic(base_size = 16) + -->
<!--   theme(axis.ticks =element_blank(), axis.text = element_blank(), plot.title = element_text(hjust=0.5)) -->
<!-- ``` -->

<!-- ```{r} -->
<!-- plotMiloReducedDim(sim_milo, milo_results, reduced_dim = "UMAP", components = c(1,2), filter_alpha = 0.1) -->
<!-- ``` -->
...
```{r, warning=FALSE, message=FALSE, echo=TRUE}
bind_rows(
  data.frame(neighborhood_size = unlist(lapply(nhoods(sim_milo), length)), sampling="random"),
  data.frame(neighborhood_size = unlist(lapply(nhoods(sim_milo_ref), length)), sampling="refined")
  ) %>%
...
for (x in seq(0.01,0.5, by = 0.05)) {
  for (i in 1:3){
    random_nh  <- nhoods(makeNhoods(sim_milo, prop=x, k=20, d=30, refined = FALSE, seed=42 + i))
    refined_nh <- nhoods(makeNhoods(sim_milo, prop=x, k=20, d=30, refined = TRUE, seed=42 + i))
    l_ran <- length(random_nh)
    l_ref <- length(refined_nh)
...
res_rand <- testNhoods(sim_milo, ~ 1 + condition, design.df = design_df, fdr.weighting = "k-distance")
...
Do I always end up with the same neighbourhoods with refined sampling?
```{r}
...
How distant are uncommon cells sampled with low proportions?
...
# test_robusteness <- function(sim_milo, prop, seeds = 2000+seq(1,5), k=20){
seeds = 2000+seq(1,5)
indexes <- sapply(seeds, function(s) unlist(nhoodIndex(makeNhoods(sim_milo, prop=0.1, k = 20, refined = T, seed=s))))
names(indexes) <- seeds

nms <- t(combn(x = seeds, m=2, simplify = TRUE))
nms <- nms[nms[,1]!=nms[,2],]
big_df <- lapply(1:nrow(nms), function(i) calc_uncommon_dist(indexes[as.character(nms[i,1])][[1]], indexes[as.character(nms[i,2])][[1]])) %>%
  imap(~ .x %>% mutate(sample_pair=.y)) %>%
  purrr::reduce(bind_rows)

all_nn_dist <- as.matrix(sim_milo@nhoodDistances) %>%
  {ifelse(.==0, NA, .)} %>%
  {.[!is.na(.)]}

first_nn_dist <- as.matrix(sim_milo@nhoodDistances) %>%
  {ifelse(.==0, NA, .)} %>%
  apply(., 1, function(x) min(x, na.rm=TRUE))

bind_rows(
  data.frame(distance=all_nn_dist, class="All NNs", sample_pair = max(big_df$sample_pair)+1),
  data.frame(distance=first_nn_dist, class="First NN", sample_pair = max(big_df$sample_pair)+2),
  big_df
) %>%
  ggplot(aes(distance, color=class, group=sample_pair)) + 
