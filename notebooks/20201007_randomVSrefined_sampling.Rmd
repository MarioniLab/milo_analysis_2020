...
```{r}
# 
# purrr::reduce(dist_p_ls, bind_rows) %>%
#   # filter(prop==0.1) %>%
#   mutate(bin=cut(distance, breaks = seq(0,16,by = 0.5), include.lowest = TRUE)) %>%
#   mutate(bin= str_remove_all(as.character(bin), "\\(|\\[|,.+") %>% as.numeric()) %>%
#   group_by(prop) %>%
#   mutate(n_tot=n()) %>%
#   ungroup() %>%
#   group_by(prop, bin) %>%
#   summarise(n=n(), n_tot=min(n_tot)) %>%
#   mutate(frac = n/n_tot) %>%
#   ggplot(aes(bin, y=frac, fill=as.factor(prop))) +
#   geom_col(position="identity") +
#   scale_fill_viridis_d() +
#   # ggplot(aes(as.factor(prop), distance, fill=as.factor(prop))) +
#   # geom_violin()
#   facet_grid(as.factor(prop) ~., scales="free_y")
```


```{r}

mean_dist <- as.matrix(sim_milo@nhoodDistances) %>%
  {ifelse(.==0, NA, .)} %>%
  {.[!is.na(.)]}
