...
top.genes <- rownames(sig.markers[order(sig.markers$t, decreasing=TRUE), ])[1:20]
bottom.genes <- rownames(sig.markers[order(sig.markers$t, decreasing=FALSE), ])[1:20]
plot.genes <- c(top.genes, bottom.genes)
gene.symbols <- gene.df[plot.genes, ]$external_gene_name
itec.nhoods <- itec.res$Nhood
pc.exprs <- sapply(plot.genes, FUN=function(G) .calculate_nhood_perc_expression(thymus.mylo, itec.nhoods, G))
pc.exprs <- as.data.frame(pc.exprs)
colnames(pc.exprs) <- gene.symbols
pc.exprs$Nhood <- rownames(pc.exprs)
pc.exprs.merge <- merge(pc.exprs, itec.res, by='Nhood')
pc.exprs.merge$logFC_rank <- rank(pc.exprs.merge$logFC)
```


```{r, fig.height=8.95, fig.width=7.95, message=FALSE, warning=FALSE}
pl_top <- pc.exprs.merge %>%
    mutate(is_signif = ifelse(SpatialFDR < 0.01, "SpatialFDR < 0.01", NA)) %>%
    ggplot(aes(logFC_rank, logFC)) +
    geom_hline(yintercept = 0, linetype=2) +
    geom_point(size=1) +
    geom_point(data=.%>% filter(!is.na(is_signif)), aes(color=is_signif), size=1.0) +
    theme_bw() +
...
pl_bottom <- pc.exprs.merge %>%
    pivot_longer(cols=gene.symbols, names_to='feature', values_to="perc_expressed") %>%
    ggplot(aes(logFC_rank, reorder(feature, logFC_rank, mean), fill=perc_expressed)) + 
    geom_tile() +
    labs(x="logFC Rank", y="Gene") +
    scale_fill_viridis_c(option="magma") +
    ggbio::theme_clear() +
    theme(axis.text=element_text(size=11, colour='black'),
          axis.title=element_text(size=12, colour='black')) +
    guides(fill=guide_colourbar(title="%Expressed"))
  
plot_grid(pl_top, pl_bottom, ncol=1, rel_heights = c(0.25,1))

ggsave("~/Dropbox/Milo/figures/AgeingThymus_iTEC_DEgenes.pdf",
       height=8.95, width=7.95, useDingbats=FALSE)
```
