...
donor_cnts <- nhoodCounts(countCells(liver_milo, samples = "liver", meta.data = data.frame(colData(liver_milo)[,c("dataset","condition",'liver')]) ))

melt(as.matrix(nhoodCounts(liver_milo)), varnames=c("Nhood", "dataset")) %>%
  mutate(donor=str_remove(dataset, "_.+")) %>%
  ggplot(aes(Nhood, value, fill=dataset)) +
  geom_col(position="fill")
```


## Run other methods for DA analysis
...
subset.nhoods <- which(str_detect(milo_res$annotation_indepth, "Endo"))
...
set.seed(42)
milo_res_endogroups <- groupNhoods(liver_milo, milo_res, max.lfc.delta = 1, overlap = 1)
...
# milo_res_endogroups %>%
#   group_by(annotation_lineage,  NhoodGroup) %>%
#   summarise(n=n()) %>%
#   arrange(n) %>%
#   mutate(NhoodGroup = factor(NhoodGroup, levels=unique(NhoodGroup))) %>%
#   mutate(annotation_lineage = factor(annotation_lineage, levels=unique(annotation_lineage))) %>%
#   ggplot(aes(annotation_lineage, NhoodGroup, fill=n)) +
#   geom_tile()
#   geom_jitter() +
...
p3 <- plotNhoodExpressionGroups(liver_milo, milo_res_endogroups, features = unique(tcell_marker_genes), 
                          subset.nhoods = milo_res_endogroups$NhoodGroup %in% c("23", "20", "18", "14"),
...
```{r}
# milo_res_endogroups$NhoodGroup <- ifelse(is.na(milo_res_endogroups$annotation_lineage) & (milo_res_endogroups$NhoodGroup %in% c("54", "70")),
#                               NA,
#                               milo_res_endogroups$NhoodGroup)
...
marker_genes_up <- marker.df %>%
  dplyr::filter(adj.P.Val_cirr < 0.05 & logFC_cirr > 0) %>%
  pull(GeneID)

marker_genes_down <- marker.df %>%
  dplyr::filter(adj.P.Val_cirr < 0.05 & logFC_uninj > 0) %>%
  pull(GeneID)
...
plot_grid(p1, p3, ncol=2, rel_widths = c(1,1)) +
# cowplot::plot_grid(p1,p2,go_chol_up,  rel_heights = c(1,1.5), ncol=1, labels = c("", "F")) +
  ggsave("~/mount/gdrive/milo/Figures/liver_v2/suppl_fig6.pdf", height = 16, width=20)
  # ggsave("~/mount/gdrive/milo/Figures/liver_v2/suppl_fig6.png", height = 16, width=20)
