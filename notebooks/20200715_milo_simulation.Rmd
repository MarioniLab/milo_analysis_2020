...
Using [`dyntoy`](https://github.com/dynverse/dyntoy), I simulate linear trajectory (no branches) with 2000 cells and 5 main states (milestones).
...
sim_gex <- as.matrix(dataset$expression)
sim_branches <- dataset$prior_information$groups_id
```

Make a SingleCellExperiment object
```{r}
coldata <- sim_branches %>% column_to_rownames("cell_id")

## Simulate conditions and replicates
...

## Make SingleCellExperiment
sim_sce <- SingleCellExperiment(assay=list(counts=t(sim_gex)), colData=coldata)
logcounts(sim_sce) <- log2(counts(sim_sce) + 1)
pca <- prcomp(t(logcounts(sim_sce)))
reducedDims(sim_sce) <- SimpleList(PCA=pca$x)
```
