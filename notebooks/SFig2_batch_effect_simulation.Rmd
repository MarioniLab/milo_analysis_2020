...
myPlotNhoodGraph <- function(x, layout="UMAP", colour_by=NA, subset.nhoods=NULL, jitter=TRUE, ... ){
  ...
  if (jitter){
    set.seed(42)
    reducedDim(x, redDim)[as.numeric(vertex_attr(nh_graph)$name),1] <- jitter(reducedDim(x, redDim)[as.numeric(vertex_attr(nh_graph)$name),1], factor=400)
    reducedDim(x, redDim)[as.numeric(vertex_attr(nh_graph)$name),2] <- jitter(reducedDim(x, redDim)[as.numeric(vertex_attr(nh_graph)$name),2], factor=400)
...
myPlotNhoodGraphDA(sim_milo_be_mnn, da_res_mnn_cond_tech, alpha = 0.1)
```
...
(pan_1 | pan_2 | pl_right) + plot_layout(widths=c(1,1.1,0.8)) +
  ggsave("~/milo_output/suppl_fig1.pdf", width = 18, height = 7.5)
```
