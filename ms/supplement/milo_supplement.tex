...
The choice of \(K\) influences the distribution of cell counts within neighbourhoods, as \(K\) represents the lower limit in the number of cells in each neighbourhood (\(\sum(y_{n,s})\)). Hence, if \(K\) is too small the neighbourhoods might not contain enough cells to detect differential abundance. On the other hand, increasing \(K\) too much might lead to ``oversmoothing'' of neighbourhoods, with the inability to detect changes in abundance. We illustrate this trade-off by testing for DA setting increasing values for \(K\) in the mouse gastrulation dataset with synthetic condition labels on 4 different populations (Suppl.Fig\ldots.A).

In order to perform DA testing with sufficient statistical power, the analyst should consider the number of experimental samples \(S\) (that will correspond to the columns in the count matrix for DA testing) and the desired minimum number of cells per neighbourhood and experimental sample. For example, having on average 5 cells per sample in each neighbourhood allows to detect n fold changes between 2 experimental conditions.
The median number of cells per sample in each neighbourhood \(\hat{y}_{ns}\) increases with the total neighbourhood size (Suppl.Fig\ldots.B), with:

\[
\hat{y}_{ns} \sim \frac{\sum_s y_{ns}}{S}
\]
Therefore a conservative approach is to select \(K \geq (S \times\) 3-5.
As the average neighbouhood size increases with \(K\),
we recommend to pick \(K\) by inspecting the histogram of neighbourhood sizes after sampling of neighbourhoods (Suppl.Fig\ldots.C). In order to have at least 3-5 cells per sample, all the neighbourhoods should be bigger than \(S \times 3-5\).
We provide the utility function \texttt{plotNhoodSizeHist} to visualize the neighbourhood size distribution as part of our R package.
